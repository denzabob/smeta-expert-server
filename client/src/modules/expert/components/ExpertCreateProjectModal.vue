  ...
  <v-dialog v-model="open" max-width="720" scrollable>
    <v-card class="expert-create-dialog">
      ...
      <v-card-actions class="expert-create-dialog__actions">
        ...
        <v-btn color="primary" variant="flat" prepend-icon="mdi-plus" @click="createProject">Создать проект</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>
...
import { expertDirections, expertWorkTypes } from '../mock/expertMockData'
...
async function createProject() {
  ...
  const projectId = draft.direction === 'Строительно-техническое' ? 'demo-construction' : 'demo-commodity'
  open.value = false
  await router.push({ name: 'expert-project-overview', params: { projectId } })
}
</script>
