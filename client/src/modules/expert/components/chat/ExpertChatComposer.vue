<template>
  <div class="expert-composer">
    <div class="expert-composer__contexts" aria-label="Контекст следующего запроса">
      ...
      <v-btn v-else size="small" variant="tonal" prepend-icon="mdi-folder-multiple-outline" @click="$emit('select-whole-project')">Выбрать весь проект</v-btn>
    </div>
...
.expert-composer__contexts { display: flex; gap: 6px; width: min(820px, 100%); margin: 0 auto 7px; overflow-x: auto; scrollbar-width: thin; white-space: nowrap; }
...
@media (max-width: 600px) { .expert-composer { padding: 8px 58px 8px 8px; } .expert-composer__contexts { margin-bottom: 6px; } .expert-composer__box { grid-template-columns: auto minmax(80px, 1fr) auto; border-radius: var(--md-sys-shape-corner-large); } .expert-composer__mode { display: none; } .expert-composer__hint { display: none; } }
</style>
