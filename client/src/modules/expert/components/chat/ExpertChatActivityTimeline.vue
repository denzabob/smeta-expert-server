...
const label = computed(() => {
  ...
  if (activity.code === 'analysis.materials.prepare') return 'Подготавливаю материалы к анализу…'
  if (activity.code === 'analysis.material.started') return activity.detail ? `Анализирую ${activity.detail}…` : 'Анализирую материал…'
  if (activity.code === 'analysis.material.completed') return 'Материал проанализирован'
  if (activity.code === 'analysis.material.failed') return 'Материал не удалось проанализировать'
  if (activity.code.startsWith('analysis.compare.')) return 'Сопоставляю сведения…'
  if (activity.code === 'analysis.final.started') return 'Формирую общий вывод…'
  if (activity.code.startsWith('analysis.')) return 'Подготавливаю анализ…'
