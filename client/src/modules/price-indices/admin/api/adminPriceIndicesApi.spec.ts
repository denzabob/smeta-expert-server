...
describe('adminPriceIndicesApi', () => {
  it('uses public UUID paths for preview/import lifecycle endpoints', async () => {
    ...
    await api.retryPreview('preview-uuid'); await api.startImport('file-uuid', 'preview-uuid'); await api.getImport('import-uuid')
    ...
    expect(client.post).toHaveBeenCalledWith('/api/indices/admin/source-files/file-uuid/imports', { preview_public_id: 'preview-uuid' })
